...
import MyTypes
  ( Error (offsetx, offsety, weightdenom, weightnum),
...
pixelToGrayscale (Pixel red green blue) =
  Pixel
    { r = new_value,
      g = new_value,
      b = new_value
  ...
  where
    new_value = round ((fromIntegral red) * 0.3 + (fromIntegral green) * 0.59 + (fromIntegral blue) * 0.11)

grayscale :: Image -> Image
grayscale (Image mrows mcols mcontent) =
  Image
    { rows = mrows,
      cols = mcols,
...
updateMatrix m value r c
  | r >= length m || c >= (length $ head m) || c < 0 = m
  | otherwise =
    take r m
